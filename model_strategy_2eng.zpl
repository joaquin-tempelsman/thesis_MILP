...
set stage_1_sell_periods_available := { 6, 7, 8 };
set stage_2_and_3_periods_available := { 16, 17, 18, 30, 31, 32, 33, 34, 35, 36 };

# Binary variable allowing sales when there are no births for constraints heuristic_sales_1 and heuristic_sales_2
var k[T] binary;

# Binary variable allowing transfers when there are no births for constraint heuristic_transfers
var l[T] binary;

# 30% births to sales at stage 1
subto heuristic_sales_1: forall <t> in august_periods with t + 6 <= max_periods:
    n[t,2] * 0.3 == sum <v> in stage_1_sell_periods_available with t+v <= max_periods: y[t+v,v,2] * k[t];

# 30% births to sales at stage 2 and 3
subto heuristic_sales_2: forall <t> in august_periods with t + 16 <= max_periods:
    n[t,2] * 0.3 == sum <v> in stage_2_and_3_periods_available with t+v <= max_periods: y[t+v,v,2] * k[t];

# 40% births assign to class 3
subto heuristic_transfers: forall <t> in august_periods with t + 11 <= max_periods:
