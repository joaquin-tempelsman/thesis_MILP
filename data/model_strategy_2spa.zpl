...
# ventas max por mes virtual_venta_max
param venta_max := read "model_inputs/parametros.dat" as "2n" skip 3 use 1;
...
# costo fijo por operar una venta en ese periodo para c1 y c2
param SELL_FIX_COST_c1c2 := read "model_inputs/parametros.dat" as "2n" skip 8 use 1;
...
# Funcion objetivo
maximize fobj: sum <t,e,c> in T*E*C: (y[t,e,c] * precio[t,e,c] - x[t,e,c] * costo[t,e,c] - s[t] * SELL_FIX_COST_c1c2);
...
subto venta_minima: forall <t> in T:
    sum <e,c> in E*C with c !=3: y[t,e,c]  >= venta_min * s[t];

#venta maxima si hay ventas, o cero si s[t] vale 0
subto venta_maxima: forall <t> in T:
    sum <e,c> in E*C with c != 3: y[t,e,c]  <= venta_max * s[t];
...
subto maintain_c3_bigger_or_equal_end_of_period_bin:
    if MANTAIN_C3_STOCK == 1 then
        sum <e> in E: x[0, e, 3] <= sum <e> in E: x[max_periods, e, 3]
    end;

subto maintain_c3_bigger_or_equal_end_of_period_young:
    if MANTAIN_C3_STOCK == 1 then
...
subto nacimientos_c1: forall <t> in meses_agosto_si:
    n[t,1]   ==  sum <e> in E with e >= 24: x[t,e,3] * 0.465;

subto nacimientos_c2: forall <t> in meses_agosto_si:
    n[t,2]   ==  sum <e> in E with e >= 24: x[t,e,3] * 0.465;
